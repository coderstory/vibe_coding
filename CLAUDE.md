...
1. **抽取样本**：
   - 后端：从 L3 层文件（Controller/Config/异常体系/JWT/AOP）中随机选 5 个文件
   - 前端：从 Vue 组件中随机选 5 个文件
2. **检查项**：
   - 是否存在空骨架注释（`@param`/`@return` 无描述）
   - 是否存在注释掉的代码块
   - 类级 Javadoc 是否与代码实际行为一致（漂移检测）
   - 是否满足 L0-L3 层级要求
3. **评分**：每个文件通过/未通过，计算通过率
4. **修复**：通过率 < 80% 时，创建 Issue 安排专项修复

**首次抽查范围**：v1.7 阶段 28 中补充注释的 Controller 和 Config 类。

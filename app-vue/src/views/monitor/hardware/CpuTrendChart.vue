<script lang="ts">
/**
 * CPU 使用率趋势图。ECharts LineChart 展示近 1 小时 CPU 使用率趋势。
...
async function loadTrend() {
  ...
  error.value = null
  try {
    const res = await fetchTrend('cpu', 360)
...
<template>
  <div class="cpu-trend-chart">
    <el-card shadow="never">
      <template #header>
        <span class="section-title">CPU 使用率趋势（近 1 小时）</span>
...
<style scoped>
.cpu-trend-chart { margin-bottom: 16px; }

...
### 默认语言设置
- **首选语言：中文** - AI 默认使用简体中文进行所有沟通
- **自动检测：** AI 会自动识别上下文并优先使用中文
- **强制切换：** 如检测到英文输出，AI 会立即纠正为中文

### 交互要求

1. **全程使用中文沟通** - 所有输出必须使用中文，包括回复内容、代码注释、错误信息、文档描述
2. **禁止输出英文** - 除非：用户明确要求、代码本身是英文、技术术语无公认中文翻译
3. **系统环境** - 当前操作系统是 Windows，调用 shell 时，**禁止使用 bash 命令**，而是使用 **PowerShell 7 命令**：`pwsh -Command "your command"`
4. **回复风格** - 简洁直接，不确定的主动询问
5. **AI 框架** - 优先使用 Superpowers-ZH 技能框架进行中文优化
6. **模型限制** - 当前只能使用 MiniMax-M2.7 模型，禁止调用 gpt 系列模型

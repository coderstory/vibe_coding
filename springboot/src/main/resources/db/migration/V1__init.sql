...
-- 创建数据库
CREATE
DATABASE IF NOT EXISTS admin_system DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
(
  50
) NOT NULL COMMENT '操作类型(LOGIN/LOGOUT/新增/编辑/删除)',
  target_type VARCHAR
(
  50
) COMMENT '目标对象类型',
...
(
  20
) DEFAULT 'light' COMMENT '主题(light/dark)',
  create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  UNIQUE KEY uk_user_id
(
  user_id
)
...
VALUES (1, '超级管理员', 'ADMIN', '拥有所有权限');

-- 插入初始菜单数据（系统管理菜单）
INSERT INTO sys_menu (id, parent_id, name, path, icon, sort_order)
VALUES (1, 0, '系统管理', '', 'Setting', 1),
